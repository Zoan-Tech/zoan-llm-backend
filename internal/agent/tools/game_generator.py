...
from external.app_preview.model import BuildStatus
from external.minio.bucket.games import service as minio_games_service
...
def search_knowledge_hub(
    ...
    results = qdrant_library_service.client.query_points(
        collection_name=qdrant_library_service.collection_name,
        query=text_vector,
        using="text",
...
def init_or_load_game_source() -> str:
    ...
    folder_count = minio_games_service._count_folders_in_thread(thread_id)
    if folder_count > 0:
        ...
        minio_games_service._download_game_version(source_path, data_path, source_type=SOURCE_TYPE)
        ...
        # success = minio_games_service._download_game_version(source_path, data_path, source_type="folder")
        response = (
...
def build_source(
    ...
    try:
        ...
        minio_games_service._upload_source_game(
