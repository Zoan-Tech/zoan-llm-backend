...
class MinioService:
    ...
    def _ensure_bucket_exists(self):
        ...
        logger.info(f"{logger_prefix} Ensuring bucket exists: {self.config.bucket}")
        try:
            if not self.client.bucket_exists(self.config.bucket):
                self.client.make_bucket(self.config.bucket)
        except S3Error as e:
            logger.error(f"{logger_prefix} Error creating bucket {self.config.bucket}: {e}")
            raise
    ...
    async def upload_folder_with_prefix(self, folder_path: str, prefix: str) -> str:
        ...
        """
        logger.info(f"{logger_prefix} Starting folder upload: {folder_path} with prefix: {prefix}")
        try:
            ...
            logger.error(f"{logger_prefix} Error uploading folder to MinIO: {e}")
            raise
    ...
    def upload_file(self, local_file_path: str, object_name: str) -> str:
        ...
        """
        logger.info(f"{logger_prefix} Starting file upload: {local_file_path} as {object_name}")
        try:
            self.client.fput_object(
                self.config.bucket,
                object_name,
            ...
            logger.error(f"{logger_prefix} Error uploading file to MinIO: {e}")
            raise
    ...
    def get_file_url(self, object_name: str, expires_in_days: int = 7) -> str:
        ...
        """
        logger.info(f"{logger_prefix} Generating presigned URL for: {object_name}")
        try:
            ...
            logger.error(f"{logger_prefix} Error generating presigned URL: {e}")
            raise
    ...
    def list_objects(self, prefix: str = None) -> list:
        ...
        """
        logger.info(f"{logger_prefix} Listing objects with prefix: {prefix}")
        try:
            ...
            logger.error(f"{logger_prefix} Error listing objects: {e}")
            raise